...
class user_service:
    ...
    def insert_static_value(user_list):
        ...
        cur = conn.cursor()
        cur.execute("""
                    insert into user(id, email, password, user_name, course ) values ( %s, %s, %s, %s, %s)
                    """,
                    (1111, 'test@test.com', 'password', 'test', 'AIML'))
    ...
    def create_user(self, user):
        ...
        cur.execute("""
                insert into user(id, email, password, user_name,course ) values ( %s, %s, %s, %s, %s)
                """,
                    (user.id, user.email, user.password, user.user_name, user.course))
        print(conn.insert_id())
        conn.commit()
